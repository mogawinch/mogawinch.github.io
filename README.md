...
The site is a **gaming review site** - honest, down-to-earth reviews scored on the value a game delivers for its price. Every page is prerendered to real static HTML at build time, so content is visible to search engines and social-media crawlers with no client-side rendering required.
...
- Game reviews (`/reviews/games`) with search and pagination
- Per-article page prerendered with correct `<title>`/Open Graph tags baked in
- `sitemap.xml` / `robots.txt` generated at build time
- A `/projects` page for the Pokémon: Petalburg Dynasty romhack, with download links and trailer
