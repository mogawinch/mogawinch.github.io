...
const STATIC_ROUTES = ['/', '/about', '/projects', '/privacy', '/terms', '/reviews/games'];
...
// Blazor pages, which only ever listed articles referenced here). Files
// that aren't listed in index.json never appear on the site.
function loadArticles() {
  ...
  _articles = Object.entries(modules)
    .map(([file, mod]) => {
      ...
      const route = `/reviews/games/${slug}`;
